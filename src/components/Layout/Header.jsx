...
<img src="https://wblhguozbmfnrzoolids.supabase.co/storage/v1/object/public/nova%20logo/Nova%20(20).png" alt="НОВА.мн" className={styles.logoImg}/>
</a>
<nav className={styles.nav}>
{CATS.map(cat=>(
<button key={cat.slug} className={styles.navItem+(activeCategory===cat.slug?' '+styles.navActive:'')} onClick={()=>onCategoryChange(cat.slug)}>
{cat.label}
</button>
))}
</nav>
<div className={styles.actions}>
<a href="https://facebook.com" target="_blank" rel="noreferrer" className={styles.fbBtn}>f 65,000 дагагч</a>
{searchOpen?(
<form onSubmit={handleSearch} className={styles.searchForm}>
<input autoFocus value={searchVal} onChange={e=>setSearchVal(e.target.value)} placeholder="Хайх..." className={styles.searchInput}/>
</form>
):(
<button className={styles.searchBtn} onClick={()=>setSearchOpen(true)}>Хайх</button>
)}
